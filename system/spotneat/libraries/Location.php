 ...
 *
 * @category       Libraries
 * @package        SpotnEat\Libraries\Location.php
...
class Location {
	...
	public function initialize($local_info = array()) {
		...
		if (!$is_loaded) {
			if (!isset($local_info['location_id']) OR empty($this->locations[$local_info['location_id']])) {
				...
				$this->order_type = (isset($local_info['order_type'])) ? $local_info['order_type'] : ($location['offer_delivery'] === '1' ? '1' : '2');

				$this->delivery_fee = (isset($location['delivery_fee'])) ? $location['delivery_fee'] : '0';
	...
	public function getDescriptionAR() {
		return $this->local_info['description_ar'];
	}
	...
	public function getCityAR() {
		return $this->local_info['location_city_ar'];
	}

	public function getState() {
		return $this->local_info['location_state'];
	}
	public function getStateAR() {
		return $this->local_info['location_state_ar'];
	}
	...
	public function getrewarddetails() {
		$reward_details = array(
			'reward_status'      => $this->local_info['reward_status'],
			'rewards_value'      => $this->local_info['rewards_value'],
			'rewards_enable'      => $this->local_info['rewards_enable'],
			'point_value'        => $this->local_info['point_value'],
			'point_price'        => $this->local_info['point_price'],
			'minimum_price'      => $this->local_info['minimum_price'],
			'rewards_method'     => $this->local_info['rewards_method'],
			'maximum_amount'     => $this->local_info['maximum_amount']
		);

		return $reward_details;
	}

	public function getAddressAR($format = TRUE) {
		$location_address = array(
			'address_1'      => $this->local_info['location_address_1_ar'],
			'address_2'      => $this->local_info['location_address_2_ar'],
			'city'           => $this->local_info['location_city_ar'],
			'state'          => $this->local_info['location_state_ar'],
			'postcode'       => $this->local_info['location_postcode']
		);
	...
	public function formatAddressAR($address = array(), $format = TRUE) {
		if (!empty($address) AND is_array($address)) {
			$location_address = array(
				'address_1_ar'      => $address['location_address_1_ar'],
				'address_2_ar'      => $address['location_address_2_ar'],
				'city_ar'           => $address['location_city_ar'],
				'state_ar'          => $address['location_state_ar'],
				'postcode'       => $address['location_postcode']
			);
	...
	public function getReservationStaytime() {
    	return (!empty($this->local_info['reservation_stay_time'])) ? $this->local_info['reservation_stay_time'] : $this->CI->config->item('reservation_stay_time');
	}
...
/* Location: ./system/spotneat/libraries/Location.php */