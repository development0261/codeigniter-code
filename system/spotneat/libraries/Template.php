 ...
 *
 * @category       Libraries
 * @package        SpotnEat\Libraries\Template.php
...
//			$this->CI->output->cache($this->CI->config->item('cache_time'));
	...
	protected function _buildNavMenu($nav_menu = array(), $has_child = 0) {
		...
		$out = '';
		/*echo "<pre>";
		print_r($nav_menu);exit;*/
		//$view_menus = $this->CI->user->get($perm.'.Access')
		if($this->CI->user->getStaffId() != 11){
			$view_menus = $this->CI->user->getViewMenus($this->CI->user->getStaffId());
			// $view_menus = $this->CI->user->getViewMenus($this->CI->user->getId());
		}
		// echo $this->CI->user->getStaffId();
		// exit;
		// echo "<pre>";
		// print_r($nav_menu);exit;
			...
			if (isset($menu['permission'])) {
				...
				foreach ($permission as $perm) {
					...
					if($this->CI->user->getStaffId() != 11){
						...
						if($permitted[strtolower($perm)]){
							if($view_menus[$check[1]] == "on"){
								$permitted[strtolower($perm)] = 1;
							}else{
								$permitted[strtolower($perm)] = 0;
							}
						}
...
/* Location: ./system/spotneat/libraries/Template.php */