...
/**
 * SpotnEat Calendar Class
 *
 * @category       Libraries
 * @package        SpotnEat\Libraries\TI_Calendar.php
...
class TI_Calendar extends CI_Calendar {
	...
	public function generate($year = '', $month = '', $data = array())
	{
		...
		if ($this->show_next_prev === TRUE)
		{
            ...
            $out .= str_replace('{previous_url}', $this->next_prev_url .'filter_year='. $adjusted_date['year'].'&filter_month='.$adjusted_date['month'].'&show=all', $this->replacements['heading_previous_cell']);
		}
		...
		if ($this->show_next_prev === TRUE)
		{
            ...
            $out .= str_replace('{next_url}', $this->next_prev_url .'filter_year='. $adjusted_date['year'].'&filter_month='.$adjusted_date['month'].'&show=all', $this->replacements['heading_next_cell']);
		}
			...
			for ($i = 0; $i < 7; $i++)
			{
				if ($day > 0 && $day <= $total_days)
				{
					...
					if (isset($data[$day]))
					{
                        ...
                        $day_url = $this->next_prev_url.'filter_year='.$year.'&filter_month='. $month .'&filter_day='. $fmt_day.'&show=all';
...
/* Location: ./system/spotneat/libraries/Calendar.php */