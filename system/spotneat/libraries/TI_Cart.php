...
/**
 * SpotnEat Shopping Cart Class
 *
 * @category       Libraries
 * @package        SpotnEat\Libraries\TI_Cart.php
...
class TI_Cart extends CI_Cart {
	...
	public function add_coupon($coupon = array()) {
		$table_price = $this->CI->session->userdata('reservation_data')['table_price'];
		...
		if ($coupon['type'] === 'P' AND is_numeric($coupon['discount']) AND $coupon['discount'] > 0) {
			$coupon['discount1'] = ($coupon['discount'] / 100) * $this->_cart_contents['cart_total'];
			$coupon['discount2'] += ($coupon['discount'] / 100) * $table_price;
			$coupon['discount'] = $coupon['discount1'];// + $coupon['discount2'];
		}
	...
	public function calculate_tax() {
		...
		if ($this->CI->config->item('tax_mode') === '1' AND $this->CI->config->item('tax_percentage')) {
			...
			$total = $this->_cart_contents['cart_total'] /* + $this->CI->session->userdata('reservation_data')['table_price']) */ + $this->_cart_totals['delivery']['amount'] - $this->_cart_totals['coupon']['amount'];
			...
			if ($this->CI->config->item('tax_menu_price') === '1') {
				...
				//$deduc_tax_amnt = round(($this->_cart_contents['cart_total'] + $this->CI->session->userdata('reservation_data')['table_price'] ) * $tax_percent  /100 , 2);
				//echo $deduc_tax_amnt;exit;
			    //$total = $total - $deduc_tax_amnt;
			...
			$location = $this->CI->Locations_model->getLocation($this->CI->session->userdata('local_info')['location_id']);
            ...
            for ($t=0;$t<count($tax_types);$t++)
            {
              if($tax_status[$t] == 1)
              {
               $all_tax += round(($total * $tax_perc[$t]) /100,2);
               $taxes[$t]['tax']  = $tax_types[$t];
               $taxes[$t]['percent'] = $tax_perc[$t];  
               $taxes[$t]['amount']	   =  round(($total * $tax_perc[$t]) /100,2) ;
               $taxes[$t]['action']		= $ignore;
               $taxes[$t]['priority'] 	= '3';

              }
            }

            $this->_cart_totals['taxes'] = $taxes;
			$this->_cart_totals['taxes']['priority'] = '3';
			$this->_cart_totals['taxes']['amount'] = $all_tax;
	...
	public function product_options_string($row_id, $split = '<br />') {
		...
		foreach ($this->product_options($row_id) as $option_id => $options) {
			foreach ($options as $option) {
				$string .= $this->CI->lang->line('text_plus') . lang_trans($option['value_name'],$option['value_name_ar']) . sprintf($this->CI->lang->line('text_option_price'), $this->CI->currency->format($option['value_price'])) . $split;
			}
		}
...
/* Location: ./system/spotneat/libraries/Cart.php */