 ...
 *
 * @category       Models
 * @package        SpotnEat\Models\Orders_model.php
...
class Orders_model extends TI_Model {
    ...
    public function getCount($filter = array(),$id='',$user='') {
        ...
        /*** User wise filter apply ***/
            if($user != ''){
                 /* Added By Elemensis */
                // if($this->user->getStaffId() == 13){
                //     $this->db->where('locations.added_by',$this->user->getId());
                // }else if($this->user->getStaffId() == 12){
                //     $this->db->where('locations.restaurant_by',$this->user->getId());
                // }else{
                //     $this->db->where('locations.added_by',$id);
                // }
                $this->db->where('locations.added_by',$id);
                $this->db->or_where('locations.restaurant_by', $id);
                /* Ended By Elemensis */
        ...
        $this->db->select('*, orders.status_id,statuses.status_code, statuses.status_name, statuses.status_color, orders.date_added, orders.date_modified');
        $this->db->from('orders');
        $this->db->join('statuses', 'statuses.status_code = orders.status_id', 'left');
    ...
    public function getList($filter = array(),$id='',$user='') {
        ...
        if ($this->db->limit($filter['limit'], $filter['page'])) {
            ...
            if ( ! empty($filter['filter_location'])) {
                $this->db->where_in('orders.location_id', $filter['filter_location']);
            }
            ...
            if ( ! empty($filter['filter_payment'])) {
                if($filter['filter_payment'] === "cod")
                {
                    $filter['filter_payment'] = "cash";
                }
            ...
            if($user != ''){
                /* Added By Elemensis */
                    ...
                    //     $this->db->where('locations.added_by',$id);
                    // }else if($this->user->getStaffId() == 12){
                    //     $this->db->where('locations.restaurant_by',$id);
                    // }
                    // else{
                    //     $this->db->where('locations.added_by',$id);
                    // }
    ...
    public function getOrder($order_id = FALSE, $customer_id = '') {
        if ( ! empty($order_id)) {
            ...
            $this->db->join('statuses', 'statuses.status_code = orders.status_id', 'left');
            // $this->db->join('locations', 'locations.location_id = orders.location_id', 'left');
            // $this->db->join('staffs', 'staffs.staff_id = locations.added_by', 'left');
    ...
    public function getLocation($location_id = FALSE) {
    ...
    public function getOrderDel($order_id = FALSE, $customer_id = '') {
        if ( ! empty($order_id)) {
            // echo $order_id;
            // exit;
            $this->db->from('orders');
    ...
    public function getReservationdetails($order_id) {
        if ($order_id) {
            $this->db->select('reservation_id,otp,id,booking_price');
    ...
    public function updateOrder($order_id = NULL, $update = array()) {
        ...
        if (is_numeric($order_id)) {
            ...
            if ($query === TRUE) {
                ...
                $status = $this->Statuses_model->getStatuscode($update['order_status']);

                if (isset($update['status_notify']) AND $update['status_notify'] === '1') {
                    $mail_data = $this->getMailData($order_id);
                    $reservation_details = $this->getReservationdetails($order_id);
                    //Send SMS
                    ...
                    if($sms_status['ext_data']['status'] == 1)
                    {
                        ...
                        $message = str_replace("{reservation_number}",$reservation_details['reservation_id'],$message);
                ...
                //if ((int) $update['old_status_id'] !== (int) $update['order_status']) {
                    ...
                    $status_update['staff_id']    = (int) $this->user->getStaffId();
    ...
    public function addOrder($order_info = array(), $cart_contents = array(),$cart_contents1 = array(),$post= array()) {
        $cart_contents['order_total']=$cart_contents1['order_total'];
        ...
        if (isset($order_info['booking_type'])) {
            $this->db->set('booking_type', $order_info['booking_type']);
        }
        ...
        if (isset($cart_contents['order_total'])) {
            $order_total =  $cart_contents['order_total'];
            $this->db->set('order_total', $order_total);
        }

        if (isset($cart_contents1['total_items'])) {
            $this->db->set('total_items', $cart_contents1['total_items']);
        }
        if (isset($cart_contents1['total_items'])) {
        ...
        if ( ! empty($order_info)) {
            ...
            if ($query AND $order_id) {
                if (isset($order_info['reward_points'])) {

                $sql = " UPDATE {$this->db->dbprefix('customers')} SET `reward_points` = `reward_points` + ".$order_info['reward_points']." WHERE `customer_id` = ".$order_info['customer_id'];
                $this->db->query($sql);
                }
                if (isset($order_info['address_id'])) {
                ...
                $this->addOrderMenus($order_id, $cart_contents1);

                $this->addOrderTotals($order_id, $cart_contents1);
                
                if ( ! empty($cart_contents1['totals']['coupon'])) {
                    
                    $this->addOrderCoupon($order_id, $order_info['customer_id'], $cart_contents1['totals']['coupon']);
                }
    ...
    public function getTaxes($ord_id=''){
                    ...
                    if ($name === $total_name AND is_numeric($total['amount'])) {
                        ...
                        if($name == "taxes")
                        {
                            ...
                            foreach ($total as $key => $value) {
                                ...
                                if(is_numeric($key))
                                {
                                    $value['tax'] = $value['tax'].' ( '.$value['percent'].'% )';
                                    
                                    $this->db->set('order_id', $order_id);
                                    $this->db->set('code', $name);
                                    $this->db->set('title', htmlspecialchars($value['tax']));
                                    $this->db->set('priority', $order_total['priority']);

                                    $this->db->set('value', $value['amount']);
    ...
    public function sendConfirmationMail($order_id) {
      ...
      //  print_r($mail_data );
        ...
        if(empty($mail_data['email'])){
           $cus_ids= $this->db->get_where('orders',array('order_id'=>$order_id))->row();
            $emails=$this->db->get_where('customers',array('customer_id'=>$cus_ids->customer_id))->row();
           $mail_data['email']= $emails->email;
            }
        ...
        if ($mail_data['location_email'] AND ($this->config->item('location_order_email') === '1' OR in_array('location', $config_order_email))) {
    ...
    public function getRewardAmount($res_id='',$order_id='')
    {
            $this->db->select('reward_used_amount');
            $this->db->from('reservations');
            if($res_id!=''){
...
/* Location: ./system/spotneat/models/orders_model.php */