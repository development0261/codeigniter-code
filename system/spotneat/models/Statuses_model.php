 ...
 *
 * @category       Models
 * @package        SpotnEat\Models\Statuses_model.php
...
class Statuses_model extends TI_Model {
	...
	public function getStatuses($for = FALSE,$type = 0) {
		$this->db->from('statuses');
	...
	public function getStatusesByFilter($for = FALSE,$permission) {
		$this->db->from('statuses');
		$this->db->order_by('status_for', 'ASC');
		$this->db->where('status', '1');
		...
		if ($permission == 'delivery') {
			$status_id = array(0,1,2,20,28);
			$this->db->where_in('status_code', $status_id);
		}
	...
	public function getStatusHistories($for, $order_id) {
		$this->db->select('status_history_id, status_history.date_added, staffs.staff_name, status_history.assignee_id, statuses.status_name, statuses.status_code, statuses.status_color, status_history.notify, status_history.comment');
		$this->db->from('status_history');
		$this->db->join('statuses', 'statuses.status_code = status_history.status_id', 'left');
	...
	public function getStatusHistory($for = FALSE, $order_id, $status_id = array()) {
		...
		if (!empty($status_id)) {
			$this->db->where_in('status_history.status_code', (array) $status_id);
		}
	...
	public function getStatuscode($status_id) {
		$this->db->from('statuses');
	...
	public function saveStatus($status_id, $save = array()) {
		...
		if (isset($save['status_code'])) {
			$this->db->set('status_code', $save['status_code']);
		}
...
/* Location: ./system/spotneat/models/statuses_model.php */