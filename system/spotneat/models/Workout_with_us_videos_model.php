...
class Workout_with_us_videos_model extends TI_Model {

	public function getCount($filter = array()) {
		$this->db->from('workout_with_us_videos');
	...
	public function getList($filter = array()) {
		...
		if ($this->db->limit($filter['limit'], $filter['page'])) {
			...
			$result =  $this->db->get('workout_with_us_videos');
	...
	function get_all_records_related_videos($search_cond, $sOrder , $sLimit )
	{
		...
		//echo $this->db->dbprefix('workout_with_us_videos') ; 
		$cond =  $search_cond.' '.$sOrder.' '.$sLimit ; 
		/* $sQuery = "
			SELECT rv.* , rvs.schedule_name,  rvm.module_name, rvmm.name
			FROM   ".$this->db->dbprefix('workout_with_us_videos')." as rv inner join  ".$this->db->dbprefix('workout_video_schedules')." as rvs on rv.workout_video_schedule_id=rvs.workout_video_schedule_id inner join ".$this->db->dbprefix('workout_video_modules')." as rvm on  rv.workout_video_module_id=rvm.workout_video_module_id 
...
$sQuery = "
			SELECT *
			FROM   ".$this->db->dbprefix('workout_with_us_videos')."  ".$cond ;
	...
	function get_all_records_related_videos_count()
	{
		...
		$sQuery = "
			SELECT *
			FROM   ".$this->db->dbprefix('workout_with_us_videos') ;
  		$query = $this->db->query($sQuery);
  		//echo $this->db->last_query() ; 
	...
	public function saveVideo( $data = array()) {
		...
		if (empty($data)) return FALSE;
			...
			$query = $this->db->insert('workout_with_us_videos',$data);
 	...
 	public function updateVideo($id, $data = array()) {
		...
		if (empty($data)) return FALSE;			 
			...
			$this->db->where('workout_with_us_video_id', $id);
			$query = $this->db->update('workout_with_us_videos',$data);
 	...
 	public function deleteVideo($id) {
			...
			$this->db->where('workout_with_us_video_id', $id);
			$this->db->delete('workout_with_us_videos');
...
/*
	...
	*/
	public function importRelatedVideoData($save = array()) {
		...
		$query = $this->db->insert('workout_with_us_videos');
		$workout_with_us_video_id = $this->db->insert_id();
		
		return $workout_with_us_video_id;
	}
