 ...
 *
 * @category       Models
 * @package        SpotnEat\Models\Maintenance_model.php
...
class Maintenance_model extends TI_Model {
	...
	public function backupDatabase($backup = array()) {
		if ( ! empty($backup)) {
			...
			$file_name = ! empty($backup['file_name']) ? $backup['file_name'] : 'spotneat-' . $timestamp;
	...
	public function restoreDatabase($backup_file) {
		...
		if (isset($file['filename']) AND strpos($file_path, 'spotneat-') !== FALSE) {
...
/* Location: ./system/spotneat/models/maintenance_model.php */