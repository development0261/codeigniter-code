 ...
 */
class Refund_model extends TI_Model {

	public function getCount($filter,$type) {
		
		if ($filter['filter_status']) {
			$this->db->where('type', $filter['filter_status']);
		}

		$this->db->from('refund');
		$this->db->where('type',$type);
		$this->db->where('staff_id',$this->user->getId());
		// if($this->user->getStaffId() != 11)
		return $this->db->count_all_results();
	}

	public function getList($filter = array(),$type='') {
		$this->db->select('*');
		...
		if ($this->db->limit($filter['limit'], $filter['page'])) {
			...
			$this->db->where('type',$type);
			$this->db->where('staff_id',$this->user->getId());
			$this->db->join('reservations','reservations.reservation_id=refund.reservation_id','left');
