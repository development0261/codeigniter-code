 ...
 *
 * @category       Models
 * @package        SpotnEat\Models\Tables_report_model.php
 * @link           http://docs.spotneat.com
 */
class Tables_report_model extends TI_Model {



	public function getReservedList($location_id, $date , $time) {
			...
			$this->db->join('reservations', 'reservations.table_id = location_tables.table_id', 'left');
			$array = array('reservations.status' => 16,'reservations.reserve_time' => $time,'reservations.reserve_date' => $date, 'reservations.location_id' => $location_id, 'locations.location_id' => $location_id);
			$this->db->where($array); 			
			$query = $this->db->get();
	...
	public function getList($filter = array(),$id='',$user='') {
		...
		if ($this->db->limit($filter['limit'], $filter['page'])) {
			...
			if (APPDIR === ADMINDIR) {
				...
				if ( ! empty($filter['filter_table'])) {
					$this->db->where('reservations.table_id', $filter['filter_table']);
				}
	...
	public function getReservation($reservation_id = FALSE, $customer_id = FALSE) {
		if ($reservation_id !== FALSE) {
			if (APPDIR === ADMINDIR) {
				$this->db->select('*, reservations.date_added, reservations.date_modified,reservations.otp, reservations.status, tables.table_id, staffs.staff_id, locations.location_id');
				$this->db->join('staffs', 'staffs.staff_id = reservations.assignee_id', 'left');
			} else {
		...
		if ($reservation_id) {
			...
			if (isset($update['notify']) AND $update['notify'] === '1') {
				...
				if($sms_status['ext_data']['status'] == 1)
				{
	                ...
	                $vendor_msg		= $ctlObj->sendSms($mail_data['staff_telephone'],$message);
                }
