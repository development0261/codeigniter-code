 ...
 */
class Delivery_fees_model extends TI_Model {

	public function deliveryFees($filter = array()) {
	
		$this->db->set('value', $this->input->post('standard_fee'));
		$this->db->where('item', 'standard_fee');		
		$return['standard_fee'] = $this->db->update('settings');

		$this->db->set('value', $this->input->post('premium_fee'));
		$this->db->where('item', 'premium_fee');		
		$return['premium_fee'] = $this->db->update('settings');
		return $return;
	}
