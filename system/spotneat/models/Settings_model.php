 ...
 *
 * @category       Models
 * @package        SpotnEat\Models\Settings_model.php
...
class Settings_model extends TI_Model {
	...
	public function defaultLang() {
			...
			$this->db->where('item', 'admin_language_id');
			$this->db->from('settings');
			$query = $this->db->get();
			$result = array();
			if ($query->num_rows() > 0) {
	...
	public function updateAdminLang($def_lang='') {
		
			$this->db->set('value',$def_lang);
			$this->db->where('item', 'admin_language_id');
...
/* Location: ./system/spotneat/models/settings_model.php */