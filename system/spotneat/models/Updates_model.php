 ...
 *
 * @category       Models
 * @package        SpotnEat\Models\Updates_model.php
...
class Updates_model extends TI_Model {
	...
	protected $endpoint = 'https://api.spotneat.com/v1';
	...
	public function update($update_type, $update_name = '', $update_version = '') {
		...
		if ($update_type === 'core') {
			...
			$filename = 'SpotnEat-' . $update_version;

			flush_output(sprintf($this->lang->line('progress_update'), 'SpotnEat'));
			flush_output($this->lang->line('progress_download_update'));
		} else {
			$extension = $this->Extensions_model->getExtension($update_name);

			$filename = 'SpotnEat-' . $update_type . '-' . $update_name;
	...
	public function installCoreUpdate($update_version, $update_file) {
		...
		if ( ! is_dir($temp_path)) {
			...
			$remove_files = array(
				'/tests',
				'/system/spotneat/logs',
				'/system/spotneat/session',
	...
	public function flushUpdatedFiles() {
		...
		$html .= '<p>'.sprintf($this->lang->line('progress_update_success'), 'SpotnEat').'</p>';
		flush_output($html, FALSE);
	}
}

/* End of file Updates_model.php */
/* Location: ./system/spotneat/models/Updates_model.php */