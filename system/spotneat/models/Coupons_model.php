...
class Coupons_model extends TI_Model {
	...
	public function getList($filter = array()) {
		...
		if ($this->db->limit($filter['limit'], $filter['page'])) {
			...
			if (isset($filter['filter_location']) AND is_numeric($filter['filter_location'])) {
				$this->db->where('location_id', $filter['filter_location']);
			}

			// if (isset($id ) AND ($id != '11')) {
	...
	public function getCouponByLocation($code, $location_id, $coupon_id) {
		$this->db->from('coupons');
	...
	public function saveCoupon($coupon_id, $save = array()) {
		...
		if (isset($save['is_fd_type_percent'])) {
			$this->db->set('is_fd_type_percent', $save['is_fd_type_percent']);
		}
		...
		if (isset($save['period_start_date'])) {
			$this->db->set('period_start_date', date("Y-m-d", strtotime($save['period_start_date'])));
		}
		
		if (isset($save['period_end_date'])) {
			$this->db->set('period_end_date', date("Y-m-d", strtotime($save['period_end_date'])));
		}
		...
		if (isset($save['is_public_access'])) {
			$this->db->set('is_public_access', $save['is_public_access']);
		}
		...
		} else if (!empty($save['location_id_staff'])) {
			$this->db->set('location_id', $save['location_id_staff']);
		} else {
			$this->db->set('location_id', '29');
		}
	...
	public function getLocationList($filter = array(),$user='') {
