 ...
 *
 * @category       Helpers
 * @package        SpotnEat\Helpers\template_helper.php
...
if ( ! function_exists('load_theme_config')) {
	...
	function load_theme_config($filename = NULL, $domain = MAINDIR) {
		...
		//print_r($user_data['staff_group_id']);exit;
		
		if($user_data['staff_group_id'] != 11 && $filename == "spotneat-blue"){
			include(ROOTPATH . "{$domain}/views/themes/{$filename}/theme_config_vendor.php");
		}else{
...
if ( ! function_exists('create_child_theme_files')) {
	...
	function get_current_language() {
		...
		$lang = $CI->Languages_model->getCurrentLanguage();
		return $lang;
...
/* Location: ./system/spotneat/helpers/template_helper.php */