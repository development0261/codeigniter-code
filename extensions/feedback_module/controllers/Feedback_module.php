...
class Feedback_module extends Main_Controller {
    ...
    public function index() {
        if ( ! file_exists(EXTPATH .'feedback_module/views/feedback_module.php')) { 								//check if file exists in views folder
            show_404(); 																		// Whoops, show 404 error page!
        }
        
        $payment = $this->extension->getPayment('feedback_module');
        ...
        return $this->load->view('feedback_module/feedback_module', $data, TRUE);
    }
    ...
    public function confirm() {
        ...
        if (!empty($order_data['payment']) AND $order_data['payment'] == 'twilio_module') { 	// check if payment method is equal to paypal
            ...
            $this->load->model('twilio_module/Paypal_model');
