...
class Reservation_module extends Main_Controller {
	...
	public function index() {
        ...
        $page_url = $this->uri->rsegment('1') == 'reservation' || $this->uri->rsegment('1') == 'local' ? page_url() : site_url('reservation');
        $data['current_url'] 			= $page_url.'?action=find_table&';
		$data['reset_url'] 				= $page_url;

        $data['find_table_action'] = 'find_table';
        
        if ($this->uri->rsegment('1') == 'local' and $_GET['action'] != '')
        { 
            if (($this->uri->rsegment('1') == 'restaurant' OR $this->uri->rsegment('1') == 'local') AND $this->input->get() AND ($response = $this->findTable()) !== FALSE) {

                if ($this->input->get('action') === 'select_time' AND $this->input->get('selected_time')) {
                    $this->session->set_userdata(array('restaurant' => $this->session->userdata('local_info')['location_id']));
        ...
        {
            if (($this->uri->rsegment('1') == 'reataurant') AND $this->input->get() AND ($response = $this->findTable()) !== FALSE) {
        ...
        foreach ($locations as $location) {
			$data['locations'][] = array(
                ...
                'table_price' => $location['first_table_price']
			);
		}
        for($g=2;$g<=15;$g++)
        {
		...
		$data['guest_numbers'] = $gues_num;//array('2', '3', '4', '5','6', '7', '8', '9', '10');
		...
		if ($this->input->get('reserve_date')) {
            ...
            $data['date']   = mdate($date_format, strtotime('', time()));
		}
        if ($this->input->get('tables')) {
            $data['tables']   = $this->input->get('tables');
            $data['current_url'] .= 'tables='. $data['tables'] .'&';
        } else {
             $data['tables']    = '1';
        }
        
        if ($this->input->get('table_price')) {
            
            $data['table_price']   = $this->input->get('table_price');
            $data['current_url'] .= 'table_price='. $data['table_price'] .'&';
        } else {
             $data['table_price']    = $this->location->getTableprice();
        }
        ...
        /*if(strtotime($opening_time) < strtotime(date('d-m-Y h:i A', time()))){
        $start_time = mdate('%H:%i', strtotime(date('d-m-Y h:i A', time())) );
        }else{
          $start_time = mdate('%H:%i', strtotime($opening_time) + $this->location->getReservationInterval() * 60);  
        }*/
        ...
        if (!empty($response['time_slots'])) {
            for ($i = 0; $i < 5; $i++) {
                if((strtotime($response['time_slots'][$i]) > (now()+600)) || (strtotime($data['date']) > $today))
                {
        ...
        /*if(!$data['location_id'])
        {
            redirect('locations?search=jeddah');
        }*/
	...
	private function findTable() {
        ...
        if ($this->validateForm() === TRUE) {
            ...
            $find['tables']                 = $this->input->get('tables');
			$find['guest_num'] 		        = $this->input->get('guest_num');
            $find['table_price']            = $this->input->get('table_price');
            $find['reserve_date']		    = mdate('%Y-%m-%d', strtotime($this->input->get('reserve_date')));
		 	$find['reserve_time']		    = mdate('%H:%i:%s', strtotime($this->input->get('reserve_time')));
		 	$find['selected_time']		    = mdate('%H:%i:%s', strtotime($this->input->get('selected_time')));
		 	$find['time_interval']		    = $this->location->getReservationInterval();
            $find['stay_time']              = $this->location->getReservationStaytime();
            $total_tables = ceil($find['guest_num'] / 5);
            $available_table_id = $this->Reservations_model->checkReservationTable($find['guest_num'],$find['location'],$find['reserve_date'],$find['reserve_time']);

            $response = $this->Reservations_model->findATable($find,$total_tables,count($available_table_id));
                
            if(count($available_table_id) < $find['tables']){               
                $this->alert->set('alert_now', $this->lang->line('alert_fully_booked'), 'reservation_module');
             }else if ($response === 'NO_ARGUMENTS') {
                ...
                $reserve_time = strtotime($this->input->get('reserve_time')) + $this->location->getReservationInterval() * 60;
                $selected_time = strtotime($this->input->get('selected_time')) + $this->location->getReservationInterval() * 60;
                $this->findTableNext($reserve_time,$selected_time); 
                $this->alert->set('alert_now', $this->lang->line('alert_fully_booked'), 'reservation_module');
                ...
                if (isset($response['time_slots'])) {
                    $selected_time = mdate('%H:%i', strtotime($this->input->get('reserve_date') .' '. $find['reserve_time']));

                    //if (in_array($selected_time, $response['time_slots'])) {
                        $response['table_found'] = array_shift($response['table_found']);
                        $this->session->set_tempdata('reservation_data', array_merge($find, $response), 1200);
                    /*} else {
                        $this->alert->set('alert_now', $this->lang->line('alert_fully_booked'), 'reservation_module');
                        return FALSE;
                    }*/
    ...
    private function findTableNext($reserve_time,$selected_time) {
        ...
        if ($this->validateForm() === TRUE) {
            ...
            $find['stay_time']              = $this->location->getReservationStaytime();
            $find['reserve_date']           = mdate('%Y-%m-%d', strtotime($this->input->get('reserve_date')));
            $find['reserve_time']           = mdate('%H:%i:%s', strtotime($reserve_time)+2700);
            $find['selected_time']          = mdate('%H:%i:%s', strtotime($selected_time) + 2700);
            
            $total_tables = ceil($find['guest_num'] / 5);
            $available_table_id = $this->Reservations_model->checkReservationTable($find['guest_num'],$find['location'],$find['reserve_date'],$find['selected_time']);
    ...
    public function _validate_date($str) {
        $time = strtotime($str.' '.$this->input->get('reserve_time'));
        if ($time < time()) {
    ...
    public function get_timings()
    {
        ...
        $reservation_times = time_range($start_time, $closing_time, $this->location->getReservationInterval()); 
        $options = '<select name="reserve_time" id="time" class="class-select2">';
        if (!empty($reservation_times)) {
            foreach ($reservation_times as $key => $value) {  // loop through delivery times
                if(strtotime($value) > (now()+600) || $date > $today)
                {
                    $data_reservation_times[$value] = mdate($time_format, strtotime($value));
                }
            }
            foreach ($data_reservation_times as $key => $value) 
            {
                if ($value == $time) { 
