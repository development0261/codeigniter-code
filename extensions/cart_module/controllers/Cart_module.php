...
// echo
		...
		if ( ! $this->input->is_ajax_request()) {
		...
		else if ($this->location->hasSearchQuery()) {
			...
			// print_r($menu_data['menu_price']);
			// exit;
			$menu_price = number_format($menu_data['menu_price'],2);
			// echo $menu_price;
			// exit;
			...
			$cart_data = array(																// create an array of item to be added to cart with id, name, qty, price and options as keys
				...
				'name_ar'   	=> $menu_data['menu_name_ar']=='' ? $menu_data['menu_name'] : $menu_data['menu_name_ar'],
				'qty'    		=> $quantity,
				'price'  		=> $price,
	...
	public function options() {																	// _updateModule() method to update cart
		...
		$data['menu_name_ar'] 			= $menu_data['menu_name_ar']=='' ? $menu_data['menu_name'] : $menu_data['menu_name_ar'];
		$data['menu_price'] 			= $this->currency->format($menu_data['menu_price']);
		$data['description'] 			= $menu_data['menu_description'];
		$data['description_ar'] 			= $menu_data['menu_description_ar']=='' ? $menu_data['menu_description'] : $menu_data['menu_description_ar'];
	...
	public function getCart($module = array(), $data = array(), $is_mobile = FALSE) {
		...
		$permalink = $this->permalink->getPermalink('location_id='.$this->location->getId());
		...
		if ($rsegment === 'checkout' AND $checkout_step === 'two') {
			$data['button_order'] = '<a onclick="$(\'#checkout-form\').submit();"><button class="btn btn-order btn-primary btn-block btn-lg" >' . $this->lang->line('button_confirm') . '</button></a>';
		} else if ($rsegment == 'checkout') {
			$data['button_order'] = '<a  onclick="$(\'#checkout-form\').submit();"><button class="btn btn-order btn-primary btn-block btn-lg">' . $this->lang->line('button_payment') . '</button></a>';
		} else {
			$data['button_order'] = '<a href="' . base_url() . 'local/'.$permalink['slug'].'?action=checkout&"><button  class="btn btn-order btn-primary btn-block btn-lg">' . $this->lang->line('button_order') . '</button></a>';
		}
		//print_r($_SESSION);exit;
		if ($this->location->isClosed() OR ! $this->location->checkOrderType()) {
			//echo "hello";exit;
			$data['button_order'] = '<a href="' . base_url() . 'local/'.$permalink['slug'].'?action=checkout&"><button class="btn btn-default btn-block btn-lg"><b>' . $this->lang->line('button_confirm') . '</b></button></a>';
		}
		...
		if ($cart_contents = $this->cart->contents()) {															// checks if cart contents is not empty
			foreach ($cart_contents as $row_id => $cart_item) {								// loop through items in cart
				...
				if (($alert_msg = $this->cart_module_lib->validateCartMenu($menu_data, $cart_item)) === TRUE) {
					...
					$name_ar = (strlen($cart_item['name_ar']) > 25) ? strtolower(substr($cart_item['name_ar'], 0, 25)) .'...' : strtolower($cart_item['name_ar']);
					...
					$data['cart_items'][] = array(
						...
						'name_ar' 				=> $name_ar =='' ? $cart_item['name'] : $name_ar,
            ...
            $table_price = $this->session->userdata('reservation_data')['table_price'];

            $data['cart_totals'] = $this->cart_module_lib->cartTotals($table_price);
		}
		...
		if (!$is_mobile AND $fixed_cart === '1' AND $rsegment !== 'checkout') {
			...
			$data['fixed_cart'] = 'id="cart-box-affix" data-spy="" data-offset-top="'.$fixed_top_offset.'" data-offset-bottom="'.$fixed_bottom_offset.'"';
		}
		if ($is_mobile AND $fixed_cart === '1' AND $rsegment !== 'checkout') {
			$fixed_top_offset = '0';
			$fixed_bottom_offset = '0';
			$data['fixed_cart'] = 'id="cart-box" data-spy="" data-offset-top="'.$fixed_top_offset.'" data-offset-bottom="'.$fixed_bottom_offset.'"';

		}
		
		if($rsegment === 'checkout' || $_GET['menu_page']) { $data['is_checkout'] = TRUE; } else { $data['is_checkout'] = FALSE;}
