...
class Categories_module extends Main_Controller {
	...
	public function index() {
		...
		$results = $this->Categories_model->getCategories(0,'',$location_id); 			
		...
		foreach (sort_array($results) as $result) {															// loop through menu categories array
			...
			if (!empty($result['child_id'])) {
				...
				if (!empty($result['sibling_id'])) {
					$sibling = $this->Categories_model->getCategories($result['child_id'],'',$location_id);							// retrieve all menu categories from getCategories method in Menus model
				...
				$children = $this->Categories_model->getCategories($result['category_id'],'',$location_id);							// retrieve all menu categories from getCategories method in Menus model
			...
			$data['categories'][$result['category_id']] = array( 														// create array of category data to pass to view
				...
				'category_name_ar'	=>	$result['name_ar']=='' ? $result['name'] : $result['name_ar'],
	...
	protected function categoryTree($categories, $mix_it_up, $is_child = FALSE) {
		...
		if (!$is_child) {
			$tree .= '<li class="list-group-item "><a class="" href="" style="text-transform: uppercase;"><i class="fa fa-angle-right"></i>'.$this->lang->line('text_show_all').'</a>';
		}
		...
		if ( ! empty($categories)) {
			foreach ($categories as $category) {
				...
				$catname = lang_trans($category['category_name'] , $category['category_name_ar']) ;

				if (!empty($category['children'])) {
					$tree .= '<li class="list-group-item"><a'.$attr.'><i class="fa fa-angle-right"></i>' . $catname . '</a>';
					$tree .= $this->categoryTree($category['children'], $mix_it_up, TRUE);
					$tree .= '</li>';
				} else {
					$tree .= '<li class="list-group-item"><a'.$attr.'><i class="fa fa-angle-right"></i>' . $catname	 . '</a></li>';
				}
			}
		}
