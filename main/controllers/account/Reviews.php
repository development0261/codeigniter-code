...
class Reviews extends Main_Controller {
	...
	public function view() {
		...
		$data['location_name_ar'] 	= $result['location_name_ar'];
		...
		$data['date'] 				= date("h:i a - jS F Y", strtotime($result['date_added']));
	...
	public function add() {
		...
		if ($this->Reviews_model->checkReviewed($this->uri->rsegment(3), $this->uri->rsegment(4), $this->customer->getId())) {
			$this->alert->set('success', $this->lang->line('alert_review_duplicate'));
  			redirect('account/reviews');
		}

		$cus_id = $this->customer->getId();
		$obj =  $this->Reviews_model->getreservationID($this->uri->rsegment(4),$cus_id);
		$this->load->model('Statuses_model');
		if ($this->uri->rsegment(3) === 'reservation') {
			$status_exists = $this->Statuses_model->statusExists('reserve',$obj->id, $this->config->item('confirmed_reservation_status'));
		} else {
			$status_exists = $this->Statuses_model->statusExists('order',$obj->id, $this->config->item('completed_order_status'));
		}
		...
		if ($this->input->post('rating')) {
			...
			$data['rating'] = array('quality' => '0',  'service' => '0');
		}
	...
	private function _addReview() {
		...
		if ($this->validateForm() === TRUE) {
			...
			$add['sale_id'] 			= $this->uri->rsegment(4);
