...
class Login extends Main_Controller {
	...
	public function index() {
		if ($this->customer->islogged()&&(!empty($this->session->userdata('cart_content')))) { 														// checks if customer is logged in then redirect to home page.
		 //$this->session->userdata('redirect_url');
			redirect( $this->session->userdata('redirect_back_url'));
		...
		if ($this->input->post()) {																// checks if $_POST data is set
			if ($this->validateForm() === TRUE) {
				...
				if ($this->customer->login($email, $password) === FALSE) {						// invoke login method in customer library with email and password $_POST data value then check if login was unsuccessful
    			...
    			else if ($this->customer->login($email, $password) === "disabled") {						// invoke login method in customer library with email and password $_POST data value then check if login was unsuccessful
					$this->alert->set('alert', $this->lang->line('alert_inactive_status'));
    					redirect(current_url());
    			}
                    ...
                    log_activity($this->customer->getId(), 'logged in', 'customers', get_activity_message('activity_logged_in',
                 	...
                 	$red_url = $this->session->userdata('redirect_url');
                    if($red_url!=''){
                    	redirect($red_url);
                    }elseif ($redirect_url = $this->input->get('redirect')) {
