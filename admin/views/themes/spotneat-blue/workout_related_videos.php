	...
	<div class="col-md-12">
		<div class="panel panel-default panel-table">
			<div class="panel-heading">
				<h3 class="panel-title">Workout Related Videos</h3>
			...
			<form role="form" id="list-form" accept-charset="utf-8" method="POST" action="<?php echo current_url(); ?>">
				...
				<div class="col-md-12">
						...
						<select name="search_column" id="search_column" class="form-control" >
							...
							<option value="is_paid">is_paid</option>
...
$(document).ready(function(){	
	...
	var VideoData = $('#VideoList').DataTable({
		...
		"order":[],
		"ajax":{
			url:"<?php echo  base_url() ?>workout_related_videos/actionlist",
			type:"POST",
/*			 
...
/*
$("div.toolbar").html('<div class="col-md-4">Category<select name="category" id="category"  class="form-control" ><option value="">All </option><?php foreach ($workoutVvideoModules as $key => $value) { ?><option value="<?php echo  $value->workout_video_module_id  ?>"><?php echo $value->module_name?> </option><?php }?></select></div><div class="col-md-4">Category<select name="category" id="category"  class="form-control" ><option value="">All </option><?php foreach ($workoutVvideoModules as $key => $value) { ?><option value="<?php echo  $value->workout_video_module_id  ?>"><?php echo $value->module_name?> </option><?php }?></select></div><div class="col-md-3">Category<select name="category" id="category"  class="form-control" ><option value="">All </option><?php foreach ($workoutVvideoModules as $key => $value) { ?><option value="<?php echo  $value->workout_video_module_id  ?>"><?php echo $value->module_name?> </option><?php }?></select></div>');
...
*/
	...
	$("#VideoList").on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				$('#empId').val(data.workout_related_video_id);
				...
				$('#week').val(data.week);
				//$('#workout_related_video_id').val(data.workout_related_video_id);
	...
	$("#VideoList").on('click', '.clone', function(){
		var empId = $(this).attr("id");
		var action = 'getVideo';
		$.ajax({
			url:"<?php echo  base_url() ?>workout_related_videos/actionlist",
			...
			success:function(data){
				...
				$('#VideoCloneModal').modal('show'); 

				$('#VideoCloneForm').find('input[name="empId"]').val(data.workout_related_video_id);
				//$('#empId').val(data.workout_related_video_id);

				$('#VideoCloneForm').find('input[name="day"]').val(data.day);
				$('#VideoCloneForm').find('input[name="description"]').val(data.description);
				//$('#day').val(data.day);
				//$('#description').val(data.description);

				$('#VideoCloneForm').find('input[name="duration"]').val(data.duration);
				$('#VideoCloneForm').find('input[name="filename"]').val(data.filename);

				//$('#duration').val(data.duration);
				//$('#filename').val(data.filename);

				$('#VideoCloneForm').find('input[name="is_paid"]').val(data.is_paid);
				$('#VideoCloneForm').find('input[name="reps"]').val(data.reps);
				$('#VideoCloneForm').find('input[name="sets"]').val(data.sets);
				//$('#is_paid').val(data.is_paid);
				//$('#reps').val(data.reps);
				//$('#sets').val(data.sets);

				$('#VideoCloneForm').find('input[name="status"]').val(data.status);
				$('#VideoCloneForm').find('input[name="title"]').val(data.title);
				$('#VideoCloneForm').find('input[name="video_url"]').val(data.video_url);
				$('#VideoCloneForm').find('input[name="week"]').val(data.week);
				...
				$('#VideoCloneForm').find('input[name="workout_video_id"]').val(data.workout_video_id);
				//$('#workout_video_id').val(data.workout_video_id);
				//$('#workout_video_module_id').val(data.workout_video_module_id);

				$('#VideoCloneForm').find('input[name="workout_video_module_id"]').find('option[value="'+data.workout_video_module_id+'"]').attr('selected','selected');
				//$('select#workout_video_module_id').find('option[value="'+data.workout_video_module_id+'"]').attr('selected','selected');
				var module_value = $('#VideoCloneForm').find('input[name="workout_video_module_id"]').find(":selected").text();
				$('#VideoCloneForm').find('#select2-chosen-2').text(module_value);

				//$('#workout_video_schedule_id').val(parseInt(data.workout_video_schedule_id));
				$('#VideoCloneForm').find('input[name="workout_video_schedule_id"]').find('option[value="'+data.workout_video_schedule_id+'"]').attr('selected','selected');
				//$('select#workout_video_schedule_id').find('option[value="'+data.workout_video_schedule_id+'"]').attr('selected','selected');
				var schedule_video = $('#VideoCloneForm').find('input[name="workout_video_schedule_id"]').find(":selected").text();
				$('#VideoCloneForm').find('#select2-chosen-3').text(schedule_video);

				$('#VideoCloneForm').find('.modal-title').val("<i class='fa fa-plus'></i> Clone Video");
				$('#VideoCloneForm').find('input[name="action"]').val('cloneVideo');
				$('#VideoCloneForm').find('input[name="save"]').val('Copy');
				//$('.modal-title').html("<i class='fa fa-plus'></i> Clone Video");
				//$('#action').val('cloneVideo');
				//$('#save').val('Copy');
			}
		})
	});
	...
	$("#VideoCloneModal").on('submit','#VideoCloneForm', function(event){
		event.preventDefault();
		$('#save').attr('disabled','disabled');
		//var formData = $(this).serialize();

		var form = $("#VideoCloneForm");
		// you can't pass Jquery form it has to be javascript form object
		var formData = new FormData(form[0]);
		...
		$.ajax({
			...
			success:function(data){	
				...
				if(reseult.error==1){
					$('#VideoCloneForm').find('#alert_message_error').html('<div class="alert alert-danger alert-dismissible"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>'+reseult.error_message+'</div>');
					
					setInterval(function(){
					    $('#VideoCloneForm').find('#alert_message_error').html('');
					}, 5000);
					return false ;	
				...
				$('#VideoCloneForm')[0].reset();
				$('#VideoCloneModal').modal('hide');				
				$('#VideoCloneForm').find('input[name="save"]').attr('disabled', false);
  ...
  $(document).on('blur', '.update_row', function(){
   ...
   setInterval(function(){
    $('#alert_message').html('');
		...
		$.ajax({
			url:"<?php echo  base_url() ?>workout_related_videos/import",
			method:"POST",
			data:formData,
			processData: false,
			contentType: false,
	...
	<div id="VideoModal" class="modal fade">
       ...
       <?php  
       //$workoutVvideoModules = $this->Workout_related_videos_model->workoutVvideoModules() ; 
        //$scheduleList = $this->Workout_related_videos_model->scheduleList() ; 
        //$moduleList = $this->Workout_related_videos_model->moduleList() ; 
       //print_r($moduleList);
        ?>
	...
	<div id="VideoCloneModal" class="modal fade">
    	<div class="modal-dialog">
    		<form method="post" id="VideoCloneForm" enctype="multipart/form-data">
    			<div class="modal-content">
    				<div class="modal-header">
    					<button type="button" class="close" data-dismiss="modal">&times;</button>
						<h4 class="modal-title"><i class="fa fa-plus"></i> Clone Video</h4>
    				</div>
    				<div class="modal-body">    				 
    				...
    				<div class="modal-footer">
    					...
    					<input type="hidden" name="empId" id="empId" />
    					<input type="hidden" name="action" id="action" value="" />
    					<input type="submit" name="save" id="save" class="btn btn-info" value="Copy" />
    					<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
    				</div>
    			</div>
    		</form>
