    ...
    <div class="col-md-12">
        <div class="row wrap-vertical">
            <ul id="nav-tabs" class="nav nav-tabs">
                ...
                <li><a href="#menu-variants" data-toggle="tab"><?php echo 'Menu Variants'; ?> </a></li>
        ...
        <form role="form" id="edit-form" class="form-horizontal" enctype="multipart/form-data" accept-charset="utf-8" method="POST" action="<?php echo $_action; ?>">
            <div class="tab-content">
                <div id="general" class="tab-pane row wrap-all active">
                    ...
                    <div class="form-group">
                        <label for="input-price" class="col-sm-3 control-label"><?php echo lang('label_price'); ?><span style="color:red">*</span></label>
                        <div class="col-sm-5">
                    ...
                    <!--<div class="form-group">
                        <label for="input-name" class="col-sm-3 control-label"><?php echo lang('label_veg_nv'); ?></label>
                        <div class="col-sm-5">
                            <select name="menu_type" id="type" class="form-control">
                                <option value="veg">Vegetarian</option>
                                <option value="nonveg">Non-Vegetarian</option>
                            </select>
                            <?php echo form_error('menu_type', '<span class="text-danger">', '</span>'); ?>
                        </div>
                    </div>-->

                    <div class="form-group">
                        <label for="input-name" class="col-sm-3 control-label"><?php echo lang('label_category'); ?><span style="color:red">*</span></label>
                        <div class="col-sm-5">
                            <select name="menu_category" id="category" class="form-control">
                                ...
                                <?php foreach ($categories as $category) { ?>
                                    <?php if ($menu_category === $category['category_id']) { ?>
                                        <option selected value="<?php echo $category['category_id']; ?>" <?php echo set_select('menu_category', $category['category_id'], TRUE); ?> ><?php echo $category['category_name']; ?></option>
                                    <?php } else { ?>
                    ...
                    <div class="form-group">
                        <label for="input-name" class="col-sm-3 control-label"><?php echo lang('label_location'); ?><span style="color:red">*</span></label>
                        <div class="col-sm-5">
                            <select name="location_id" id="location_id" class="form-control">
                               <!--  <option value=""><?php echo lang('text_select_restaurant'); ?></option> -->
                                <?php foreach ($locations as $location) { ?>
                                    <?php if ($location_id === $location['location_id']) { ?>
                                        <option value="<?php echo $location['location_id']; ?>" <?php echo set_select('location_id', $location['location_id'], TRUE); ?> ><?php echo $location['location_name']; ?></option>
                                    <?php } else { ?>
                ...
                <div id="menu-options" class="tab-pane row wrap-all">
                    <div class="form-group">
                        ...
                        <div class="col-sm-3">
                            <a class="btn btn-default" target="_blank" href="<?php echo site_url('menu_options/edit');  ?>"><?php echo lang('button_add_menu_options'); ?></a>
                        </div>
                    </div>
                    <br />
                    ...
                    <div id="menu-option" class="<?php echo (!$menu_options) ? 'hide' : ''; ?>">
                        <ul id="sub-tabs" class="nav nav-tabs">
                            ...
                            <?php foreach ($menu_options as $menu_option) { ?>
                                <li><a href="#option<?php echo $option_row; ?>" data-toggle="tab"><?php echo "{$menu_option['option_name']}"; ?>&nbsp;&nbsp;<i class="fa fa-times-circle" onclick="if (confirm('<?php echo lang('alert_warning_confirm'); ?>')) {
                                    $('#sub-tabs a[rel=#option<?php echo $option_row; ?>]').trigger('click');
                                    $('#option<?php echo $option_row; ?>').remove();
                                    $(this).parent().parent().remove();
                                    $('#option<?php echo $option_row+1; ?>').addClass('active');
                                    return false; 
                                } else {
                                    return false;
                                }"></i></a></li>
                ...
                <div id="menu-variants" class="tab-pane row wrap-all">
                    <div class="form-group">
                        <label for="input-name" class="col-sm-3 control-label"><?php echo 'Menu Variant Type'; ?></label>
                        <div class="col-sm-3">
                        <form role="form" id="menu-form" accept-charset="utf-8" method="POST" action="<?php echo site_url('menus/edit?id='.$menu_id.'&vendor_id='.$vendor_id);  ?>">
                            <select name="menu_variant_type_id" id="menu_variant_type_id" class="form-control" onchange="submit_variant('<?php echo site_url('menus/edit?id='.$menu_id.'&vendor_id='.$vendor_id);  ?>',this.value)">
                                <option value="">--Select variant--</option>
                                <?php
                                foreach($menu_variants_types as $value_type){
                                ?>                                
                                <option value="<?php echo $value_type['menu_variant_type_id']; ?>" <?php echo $value_type['menu_variant_type_id'] == $menu_variant_type_id ? 'selected':''; ?>><?php echo $value_type['variant_type_name'] ?></option>
                                <?php
                                }
                                ?>
                            </select>
                        ...
                        <div class="col-sm-6">
                            <a class="btn btn-default" href="<?php echo site_url('menu_variants/edit?menu_id='.$menu_id.'&location_id='.$location_id.'&vendor_id='.$vendor_id);  ?>">Add Menu variant Type</a>

                            <a class="btn btn-default" onclick="show_variant_value_popup('<?php echo $menu_id; ?>', '<?php echo $menu_variant_type_id; ?>','<?php echo $vendor_id; ?>');" href="#">Add Menu variant Value</a>
                        </div>
                    </div>
                    <br />
                    ...
                    <div>    
                        <div id="option-content">
                            <?php if ($menu_variants) { ?>
                                    <div class="tab-pane row wrap-all">   
                                        <div class="panel panel-default panel-table">
                                            <div class="table-responsive">
                                                <table class="table table-striped table-border table-sortable">
                                                    <thead>
                                                    <tr>
                                                        <th class="col-sm-4">Variant Value</th>
                                                        <th>Variant Price</th>
                                                        <th>Variant Type</th>
                                                        <th>Is default?</th>
                                                    ...
                                                    <tbody>
                                                    <?php foreach ($menu_variants as $option_value) { ?>
                                                        <tr>                                                       
                                                            <td><?php echo $option_value['type_value_name']; ?></td>
                                                            <td><?php echo $option_value['type_value_price']; ?></td>
                                                            <td><?php echo $option_value['variant_type_name']; ?></td>
                                                            <td><?php echo $option_value['is_default']; ?></td>
                                                            <td><?php echo $option_value['status']; ?></td>
                                                            <td class="action">
                                                                <a class="btn btn-edit" title="Edit Icon" href="<?php echo site_url('menu_variants/edit?menu_id='.$menu_id.'&location_id='.$location_id.'&vendor_id='.$vendor_id.'&variant_type_id=' . $option_value['menu_variant_type_id'].'&variant_type_value_id=' . $option_value['menu_variant_type_value_id']);  ?>"><i class="fa fa-pencil"></i></a>
                                                                &nbsp;&nbsp;
                                                                <a class="btn btn-trash" title="Delete Icon" href="<?php echo site_url('menus/delete_variant?menu_id='.$menu_id.'&location_id='.$location_id.'&vendor_id='.$vendor_id.'&variant_type_id=' . $option_value['menu_variant_type_id'].'&variant_type_value_id=' . $option_value['menu_variant_type_value_id']); ?>" onclick="return confirm('Are you want to delete?');"><i class="fa fa-trash"></i></a>
                                                            </td>                                                           
                                                        </tr>
                                                    <?php } ?>
                                                    </tbody>                                                    
...
<div id="VariantValueModal" class="modal fade">
    	<div class="modal-dialog">
    		<form method="post" id="VariantValueForm" enctype="multipart/form-data">
    			<div class="modal-content">
    				<div class="modal-header">
    					<button type="button" class="close" data-dismiss="modal">&times;</button>
						<h4 class="modal-title"><i class="fa fa-plus"></i> Add Variant Value</h4>
    				</div>
    				<div class="modal-body">   		
						<div class="row">	 
                        <div class="col-md-6">
		    					<div class="form-group">
									<label for="name" class="control-label">Variant Value<span class="requied-span">*</span></label>
									<input type="text" class="form-control" id="variant_value" name="variant_value" placeholder="Enter variant value" required>		
								</div>
	    					</div>   					
	    					<div class="col-md-6" >
			    				<div class="form-group">
									<label for="address" class="control-label">Price<span class="requied-span">*</span></label>
									<input type="text" class="form-control" id="variant_price" name="variant_price" placeholder="Enter variant price" required></textarea>
								</div>
							</div>	
  						</div>
  						<div class="row">  
	    					<div class="col-md-6" >
			    				<div class="form-group">
									<label for="address" class="control-label">Is Default?<span class="requied-span">*</span></label>
									<select  class="form-control-1" class="form-control" id="is_default" name="is_default" required> 
    						...
    						<div class="col-md-6" >
			    				<div class="form-group">
									<label for="address" class="control-label-1">Is Enable?<span class="requied-span">*</span></label>	
    				...
    				<div class="modal-footer">
    					...
    					<input type="hidden" name="variant_type_id" id="variant_type_id" value="" />
...
<script type="text/javascript"><!--
    $(document).ready(function() {
     ...
     <?php if(isset($_GET['menu_variant_type_id'])) { ?>   

     $('.nav-tabs a[href="#menu-variants"]').tab('show') ;
 
     <?php } ?>

     
	$("#VariantValueModal").on('submit','#VariantValueForm', function(event){
		event.preventDefault();
		$('#save').attr('disabled','disabled');
		//var formData = $(this).serialize();
		var form = $("#VariantValueForm");
    	// you can't pass Jquery form it has to be javascript form object
    	var formData = new FormData(form[0]);

		$.ajax({
			url:"<?php echo  base_url() ?>menus/addvariantvalue",
			method:"POST",
			data:formData,
			processData: false,
			contentType: false,
			success:function(data){	
				...
				$('#VariantValueForm')[0].reset();
				$('#VariantValueModal').modal('hide');				
    ...
    function show_variant_value_popup(menu_id, variant_type_id, vendor_id){
        $('#menu_id').val(menu_id);	
        $('#variant_type_id').val(variant_type_id);	
        $('#vendor_id').val(vendor_id);		
        
        $('#VariantValueModal').modal('show');
    }
//--></script>
<script type="text/javascript"><!--
    ...
    function submit_variant(url, menu_variant_type_id){
        window.location.href = url + '&menu_variant_type_id='+menu_variant_type_id
    }
    //--></script>
<script type="text/javascript"><!--
