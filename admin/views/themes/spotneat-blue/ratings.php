					...
					<tbody>
						...
						<?php foreach ($ratings as $key => $value) { ?>
							<tr id="table-row<?php echo $table_row; ?>">
								...
								<td class="action action-one handle"><a class="btn btn-grey" onclick="confirm('<?php echo lang('alert_warning_confirm'); ?>') ? $(this).parent().parent().remove() : false;"><i class="fa fa-times-circle"></i></a></td>
								<td>
									<input type="text" name="ratings[<?php echo $table_row; ?>]" class="form-control" value="<?php echo set_value('ratings['.$table_row.']', $value); ?>" />
									<?php echo form_error('ratings['.$table_row.']', '<span class="text-grey">', '</span>'); ?>
								</td>
							</tr>
						<?php $table_row++; ?>
						<?php } ?>
