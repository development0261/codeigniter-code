...
class workout_with_us_videos extends Admin_Controller {
    ...
    public function __construct() {
        ...
        parent::__construct();
        $this->load->model(array('Workout_with_us_videos_model'));
    ...
    public function index()
    {
        ...
        $this->template->setTitle('Workout With Us Videos');
        $this->template->setHeading('Workout With Us Videos');        
 
        $data['workoutVvideoModules'] = $this->Workout_with_us_videos_model->workoutVvideoModules(); 
        $data['scheduleList'] = $this->Workout_with_us_videos_model->scheduleList() ; 
        $data['moduleList'] = $this->Workout_with_us_videos_model->moduleList() ; 

        $this->template->render('workout_with_us_videos', $data);
    }
    ...
    public function __index()
    {
        ...
        if ($this->input->get('sort_by')) {
            ...
            $filter['sort_by'] = $data['sort_by'] = 'workout_with_us_video_id';
        }
        ...
        $data['subscriptionsRec'] = $this->Workout_with_us_videos_model->getList($filter);
        ...
        $config['total_rows'] = $this->Workout_with_us_videos_model->getCount($filter);
    ...
    function updateIndiviVideo(){
       ...
       unset($data['action']) ;
       unset($data['empId']) ;
       $this->Workout_with_us_videos_model->updateVideo($_POST['empId'],$data);
    ...
    function addVideo(){
          ...
          if ($this->validateForm() === TRUE) {
               ...
               unset($data['action']) ;
               $data['image'] = $image ; 
               $this->Workout_with_us_videos_model->saveVideo($data);
               $data['error_message'] = '' ; 
               $data['error'] = 0;
               echo json_encode($data);
               exit();
    ...
    public function validateImage()
    {
       ...
       if(!empty($_FILES['image']['tmp_name'])){
            ...
            $target_dir = APPPATH.'views/uploads/workoutwithusvideos/'.$image_name;
    ...
    function getVideo(){
        //echo 'yes' ;
        $data = $this->Workout_with_us_videos_model->getVideo($_POST['empId']);
        //print_r($data);
        echo json_encode($data);
        exit(0);
    ...
    function deleteVideo(){
       ...
       $data =  $_POST;  
       $this->Workout_with_us_videos_model->deleteVideo($_POST['empId']);
    ...
    public function videoList(){     
        ...
        if(!empty($_POST["search"]["value"])){
            $search_cond .= ' where (workout_with_us_video_id  LIKE "%'.$_POST["search"]["value"].'%" ';
        ...
        //echo $search_cond ; 
        //$or_array(0=>'rv.workout_with_us_video_id',1=>'rv.title') ;

        $or_array = array(0=>'workout_with_us_video_id',1=>'title') ;
        
        if(!empty($_POST["order"])){
            $order = 'ORDER BY '.$or_array[$_POST['order']['0']['column']].' '.$_POST['order']['0']['dir'].' ';
        } else {
            $order = 'ORDER BY workout_with_us_video_id DESC ';
        }
        //echo $order ; 
        ...
        $data['subscriptionsRec'] = $this->Workout_with_us_videos_model->get_all_records_related_videos($search_cond,$order,$limit);
        ...
        foreach($data['subscriptionsRec'] as $key=>$value) {
            ...
            $empRows = array();         
            $empRows[] = $value->workout_with_us_video_id;
            $empRows[] = '<div contenteditable class="update_row" data-id="'.$value->workout_with_us_video_id.'" data-column="title">'.$value->title.'</div>'  ;
            $empRows[] = '<div contenteditable class="update_row" data-id="'.$value->workout_with_us_video_id.'" data-column="description">'.$value->description.'</div>'  ;
            //$empRows[] = $value->description ;

            //$empRows[] = $value->name;
           // $empRows[] = $value->name;
            
            $empRows[] = $this->Workout_with_us_videos_model->get_workout_video(8);
            $empRows[] = $this->Workout_with_us_videos_model->get_workout_video_module($value->workout_video_module_id);
            $empRows[] = $this->Workout_with_us_videos_model->get_workout_video_schedule($value->workout_video_schedule_id);
            //$empRows[] = $value->module_name;
            //$empRows[] = $value->schedule_name;

            //$empRows[] = $value->video_url;
            $empRows[] = '<div contenteditable class="update_row" data-id="'.$value->workout_with_us_video_id.'" data-column="video_url">'.$value->video_url.'</div>'  ;
        
            //$empRows[] = $value->image;
            //$empRows[] =  '<img src="'.base_url().'views/uploads/workoutwithusvideos/'.$value->image.' />';
            $empRows[] =  '<a target="_blank" href="'.base_url().'views/uploads/workoutwithusvideos/'.$value->image.'"> <img alt="No Image" width="50" src="'.base_url().'views/uploads/workoutwithusvideos/'.$value->image.'"/></a>';
...
// $sub_array[] = '<div contenteditable class="update" data-id="'.$row["id"].'" data-column="first_name">' . $row["first_name"] . '</div>';
            ...
            $empRows[] =  '<div contenteditable class="update_row" data-id="'.$value->workout_with_us_video_id.'" data-column="week">'.$value->week.'</div>'  ;     
            $empRows[] =  '<div contenteditable class="update_row" data-id="'.$value->workout_with_us_video_id.'" data-column="day">'.$value->day.'</div>'  ;  
            $empRows[] =  '<div contenteditable class="update_row" data-id="'.$value->workout_with_us_video_id.'" data-column="sets">'.$value->sets.'</div>'  ;  

            $empRows[] =  '<div contenteditable class="update_row" data-id="'.$value->workout_with_us_video_id.'" data-column="reps">'.$value->reps.'</div>'  ;                    
            $empRows[] =  '<div contenteditable class="update_row" data-id="'.$value->workout_with_us_video_id.'" data-column="duration">'.$value->duration.'</div>'  ;  
...
*/
            ...
            $empRows[] = '<button title="Edit" type="button" name="update" id="'.$value->workout_with_us_video_id.'" class="btn btn-warning btn-xs update"><i class="fa fa-edit"></i></button> &nbsp;&nbsp; <button title="Delete" type="button" name="delete" id="'.$value->workout_with_us_video_id.'" class="btn btn-danger btn-xs delete" ><i class="fa fa-trash"></i></button>';
       ...
       // print_r($videoData); die() ; 
        ...
        $output = array(
            ...
            "recordsTotal"      => $this->Workout_with_us_videos_model->get_all_records_related_videos_count(),
            "recordsFiltered"   => $this->Workout_with_us_videos_model->get_all_records_related_videos_with_condition($search_cond),
            "data"              =>  $videoData
        );
   ...
   function updateIndiviCategory(){
       ...
       unset($data['action']) ;
       unset($data['empId']) ;
       $this->Workout_with_us_videos_model->updateCategory($_POST['empId'],$data);
    ...
    function addCategory(){
          ...
          if ($this->validateFormCategory() === TRUE) {
               ...
               $this->Workout_with_us_videos_model->saveCategory($data);
               $data['error_message'] = '' ; 
               $data['error'] = 0;
                echo json_encode($data);
                 exit();
    ...
    function getCategory(){
        //echo 'yes' ;
        $data = $this->Workout_with_us_videos_model->getCategory($_POST['empId']);
        //print_r($data);
        echo json_encode($data);
        exit(0);
    ...
    function deleteCategories(){
       ...
       $this->Workout_with_us_videos_model->deleteCategory($_POST['empId']);
    ...
    public function CategoryList(){     
        ...
        $data['subscriptionsRec'] = $this->Workout_with_us_videos_model->get_all_records_related_videos_cateogory($search_cond,$order,$limit);
        //print_r($data['subscriptionsRec']);
       ...
       // print_r($videoData); die() ; 
        ...
        $output = array(
            "draw"              =>  intval($_POST["draw"]),
            "recordsTotal"      => $this->Workout_with_us_videos_model->get_all_records_related_videos_count_category(),
            "recordsFiltered"   => $this->Workout_with_us_videos_model->get_all_records_related_videos_with_condition_category($search_cond),
            "data"              =>  $videoData
        );
...
//////////////////////
   ...
   function updateIndiviSchedule(){
       ...
       unset($data['action']) ;
       unset($data['empId']) ;
       $this->Workout_with_us_videos_model->updateSchedule($_POST['empId'],$data);
    ...
    function addSchedule(){
          ...
          if ($this->validateFormSchedule() === TRUE) {
               ...
               $this->Workout_with_us_videos_model->saveSchedule($data);
               $data['error_message'] = '' ; 
               $data['error'] = 0;
                echo json_encode($data);
                 exit();
    ...
    function getSchedule(){
        ...
        $data = $this->Workout_with_us_videos_model->getSchedule($_POST['empId']);
        echo json_encode($data);
        exit(0);
    ...
    function deleteSchedule(){
       ...
       $this->Workout_with_us_videos_model->deleteSchedule($_POST['empId']);
    ...
    public function ScheduleList(){     
        ...
        $data['subscriptionsRec'] = $this->Workout_with_us_videos_model->get_all_records_related_videos_schedule($search_cond,$order,$limit);
        //print_r($data['subscriptionsRec']);
       ...
       // print_r($videoData); die() ; 
        ...
        $output = array(
            "draw"              =>  intval($_POST["draw"]),
            "recordsTotal"      => $this->Workout_with_us_videos_model->get_all_records_related_videos_count_schedule(),
            "recordsFiltered"   => $this->Workout_with_us_videos_model->get_all_records_related_videos_with_condition_schedule($search_cond),
            "data"              =>  $videoData
        );
                ...
                if(is_uploaded_file($_FILES['file']['tmp_name']))
                {
                    ...
                    {       
                        ...
                        if(!empty($line[1])){
                            ...
                            $csvData['workout_with_us_video_id']    = $this->Workout_with_us_videos_model->importRelatedVideoData($csvData);    
                            unset($csvData);            
