		...
		if ($this->input->post() AND $this->validateLoginForm() === TRUE) {
            if (!$this->user->login($this->input->post('user'), $this->input->post('password'))) {										// checks if form validation routines ran successfully
                ...
                log_activity($this->user->getStaffId(), 'logged in', 'staffs', get_activity_message('activity_logged_in',
                    array('{staff}', '{link}'),
                    array($this->user->getLocationName().' , '.$this->user->getUserName(), admin_url('staffs/edit?id='.$this->user->getStaffId()))
                ));
	...
	public function _check_user($str) {
	 ...
	 public function success_post(){
	 	...
	 	if($order_id)
	 	{
			...
			$input = [
				'paypal_payment/'.$customer.'/success_status' => 1,
				'paypal_payment/'.$customer.'/trans_id' => $order_id,
			];
		}
		else
		{
			...
			$input = [
				'paypal_payment/'.$customer.'/success_status' => 0,
				'paypal_payment/'.$customer.'/trans_id' => $order_id,
			];
		}
