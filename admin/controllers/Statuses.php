...
class Statuses extends Admin_Controller {
	...
	public function index() {
        ...
        $data['sort_code'] 			= site_url('statuses'.$url.'sort_by=status_code&order_by='.$order_by);
        $data['sort_type'] 			= site_url('statuses'.$url.'sort_by=status_for&order_by='.$order_by);
        $data['sort_status'] 			= site_url('statuses'.$url.'sort_by=status&order_by='.$order_by);
	...
	public function edit() {
		...
		$this->template->setButton($this->lang->line('button_icon_back'), array('class' => 'btn btn-default', 'href' => site_url('statuses'), 'title' => 'Back'));
		...
		$data['status_code'] 		= $status_info['status_code'];
	...
	public function comment_notify() {
		if ($this->input->get('status_id') != '') {
			$status = $this->Statuses_model->getStatuscode($this->input->get('status_id'));
	...
	private function validateForm() {
		...
		$this->form_validation->set_rules('status_code', 'lang:label_code', 'xss_clean|trim|required|min_length[1]|max_length[2]');
