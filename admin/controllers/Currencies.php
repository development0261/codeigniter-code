...
class Currencies extends Admin_Controller {
	...
	public function edit() {
		...
		$this->template->setButton($this->lang->line('button_icon_back'), array('class' => 'btn btn-default', 'href' => site_url('currencies'), 'title' => 'Back'));
	...
	private function _deleteCurrency() {
        if ($this->input->post('delete')) {
        	$currency_id = $this->config->item('currency_id');
        	
        	$deletes = $this->input->post('delete');
        	foreach ($deletes as $key => $delete) {
        		if($delete == $currency_id) {
        			$this->alert->set('warning', sprintf($this->lang->line('alert_currency_error'), $this->lang->line('text_deleted')));
        			return false;
        		}
        	}
