...
class Categories extends Admin_Controller {
	...
	public function index() {
		...
		if($staffid=='11'){
			...
			$AddedById= $this->input->get('id');
			$location_Info = $this->Locations_model->getRestaurantLocationDetails('restaurant_by',$this->input->get('id'));
			if($location_Info['added_by'])  $AddedById = $location_Info['added_by'];
			$results = $this->Categories_model->getList($filter,$id,$AddedById);
		}else{
			if($this->session->user_info['staff_group_id']=='12'){
				...
				$location_added_by='';
				if($location_Info['restaurant_by']){
					$location_added_by =$location_Info['added_by'];
				}
				$results = $this->Categories_model->getList($filter,$location_added_by,$staffid);
			}else{
				$results = $this->Categories_model->getList($filter,$id,$staffid);
			}
		}
		...
		foreach ($results as $result) {
			...
			$data['categories'][] = array(
				...
				'description_ar' 			=> substr(strip_tags(html_entity_decode($result['description_ar'], ENT_QUOTES, 'UTF-8')), 0, 100) . '..',
				'status'		        => ($result['status'] === '1') ? $this->lang->line('text_enabled') : $this->lang->line('text_disabled'),
				// 'alcohol_status'		=> ($result['alcohol_status'] === '1') ? $this->lang->line('text_yes') : $this->lang->line('text_no'),
		...
		$config['total_rows'] 		= count($data['categories']);//$this->Categories_model->getCount($filter,$id,$staffid);
	...
	public function edit() {
		...
		if ($this->input->post() AND $category_id = $this->_saveCategory()) {
			...
			if ($this->input->post('save_close') === '1') {
				redirect('categories?id='.$this->input->post('added_by'));
			}
		...
		if($this->input->get('vendor_id') != ""){
			...
			$added_by = $category_info['added_by'];
		}
		if(empty($added_by)){
			$added_by =$this->user->getStaffId();
		}

	/*if(isset($this->input->get('vendor_id'))){
			$added_by = $this->input->get('vendor_id');
		}else{
			$added_by = $category_info['added_by'];
		}*/
		...
		$data['name_ar'] 			= $category_info['name_ar'];
		$data['parent_id'] 			= $category_info['parent_id'];
		$data['description'] 		= $category_info['description'];
		$data['description_ar'] 		= $category_info['description_ar'];
		$data['priority'] 		    = $category_info['priority'];
		$data['status'] 		    = $category_info['status'];
		$data['alcohol_status']	    = $category_info['alcohol_status'];
		...
		$data['categories'] = array();
		$results = $this->Categories_model->getCategories('',$added_by,'');
		foreach ($results as $result) {
			$data['categories'][] = array(
				'category_id'	=>	$result['category_id'],
	...
	private function _saveCategory() {
    	if ($this->validateForm() === TRUE) {
			...
			$check = $this->Categories_model->checkCategory($this->input->post(),$this->input->get('id'));
			if($check == 1) {
				$this->alert->set('error', sprintf($this->lang->line('category_exists')));
				return 0;
			} else {
