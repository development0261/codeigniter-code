...
class Coupons extends Admin_Controller {
	...
	public function index() {
		...
		$data['sort_staff_name'] 	= site_url('coupons'.$url.'sort_by=added_by&order_by='.$order_by);
		...
		$locations_info = $this->Coupons_model->getLocationList();
		if(!empty($locations_info)){
			foreach($locations_info as $key_loc => $value_loc){
				$data['locations'][$value_loc['location_id']] = $value_loc['location_name'];
			}
		}
		
		foreach ($results as $result) {
			$staff=$this->db->get_where('staffs',array('staff_id'=>$result['added_by']))->row_array();
			$data['coupons'][] = array(
				...
				'code'			=> $result['code'],
				'type'			=> ($result['type'] === 'P') ? $this->lang->line('text_percentage') : ($result['type'] === 'F' ?$this->lang->line('text_fixed_amount'): 'Only first Order Discount'),
				'type_value'	=> $result['type'],
				'is_fd_type_percent'=> $result['is_fd_type_percent'],
				'discount'		=> ($result['type'] === 'P') ? round($result['discount']) .'%' : $result['discount'],
				'min_total'		=> $result['min_total'],
				'validity'		=> ucwords($result['validity']),
				'period_start_date'=> !empty($result['period_start_date']) && $result['period_start_date'] != '0000-00-00' ? date('F j,Y', strtotime($result['period_start_date'])) : '',
				'period_end_date'  => !empty($result['period_end_date']) && $result['period_end_date'] != '0000-00-00' ? date('F j,Y', strtotime($result['period_end_date'])) : '',
		...
		$config['total_rows'] 		= count($data['coupons']);
	...
	public function edit() {
		...
		$this->template->setButton($this->lang->line('button_icon_back'), array('class' => 'btn btn-default', 'href' => site_url('coupons'), 'title' => 'Back'));
		...
		$data['is_public_access'] 	= $coupon_info['is_public_access'];
		...
		$data['location_id'] 			= $coupon_info['location_id'];
		...
		$data['session_location_id'] = !empty($_SESSION['location_id'])?$_SESSION['location_id']:'';		

		$data['locations'] = $this->Coupons_model->getLocationList();
    ...
    private function _saveCoupon() {
    	if ($this->validateForm() === TRUE) {
            ...
            if(empty($this->Coupons_model->getCouponByLocation($this->input->post('code'), $this->input->post('location_id'),$this->input->get('id'))))
            {
	            if ($coupon_id = $this->Coupons_model->saveCoupon($this->input->get('id'), $this->input->post())) {
	                ...
	                $this->alert->set('warning', sprintf($this->lang->line('alert_duplicate_coupon'), $save_type));
	            } 
        	}
        	else {
                $this->alert->set('warning', sprintf($this->lang->line('alert_code_exist'), $save_type));
            }
    ...
    private function validateForm() {
		...
		$this->form_validation->set_rules('discount', 'lang:label_discount', 'xss_clean|trim|is_natural_no_zero|required|numeric');

		// if ($this->input->post('type') === 'P') {
		// 	$this->form_validation->set_rules('discount', 'lang:label_discount', 'less_than[100]|is_natural_no_zero|xss_clean|trim|required|numeric');
		// }

		// $this->form_validation->set_rules('min_total', 'lang:label_min_total', 'xss_clean|trim|numeric|is_natural_no_zero');
