...
class Feedback extends Admin_Controller {
	...
	public function index() {
		...
		$data['sort_name'] 			= site_url('feedback'.$url.'sort_name=username&order_by='.$order_by);
		$data['sort_type'] 			= site_url('feedback'.$url.'sort_by=feedback_type&order_by='.$order_by);
		$data['sort_location'] 			= site_url('feedback'.$url.'sort_location=location_name&order_by='.$order_by);
		$data['sort_message'] 			= site_url('feedback'.$url.'sort_message=feedback_message&order_by='.$order_by);

		$data['feedback'] = array();
		$results = $this->Feedback_model->getList($filter);
		...
		$config['base_url'] 		= site_url('feedback'.$url);
		$config['total_rows'] 		= $this->Feedback_model->getListCount($filter);
		$config['per_page'] 		= $filter['limit'];

		$this->pagination->initialize($config);

		$data['restaurants_list'] = $this->Feedback_model->getLocationList();
		$data['user_type'] = $this->user->getStaffId();
		$data['pagination'] = array(
			'info'		=> $this->pagination->create_infos(),
			'links'		=> $this->pagination->create_links()
		);

		$this->template->render('feedback', $data);
	}
