...
class Reviews extends Admin_Controller {
	...
	public function index() {
		...
		$reviews = $this->Reviews_model->getList($filter,$id);
		$data['reviews'] = array();
		...
		$config['total_rows'] 		= $this->Reviews_model->getCount($filter,$id);
	...
	public function edit() {
		...
		$this->template->setButton($this->lang->line('button_icon_back'), array('class' => 'btn btn-default', 'href' => site_url('reviews'), 'title' => 'Back'));
	...
	private function _saveReview() {
    	...
    	if ($this->validateForm() === TRUE) {
			...
			if ($review_id = $this->Reviews_model->saveReview($this->input->get('id'), $this->input->post())) {
				...
				$review_count = $this->Reviews_model->getTotalLocationReviews($this->input->post('location_id'));
				$review_total = $this->Reviews_model->GetReviewdata($this->input->post('location_id'));
				/** To get average value **/
				$avg = $review_total / (2*$review_count);
				$avg = number_format($avg,1);

				/** To convert of 5 **/
				$this->Reviews_model->updatelocRating($this->input->post('location_id'),$avg);
                ...
                log_activity($this->user->getStaffId(), $save_type, 'reviews', get_activity_message('activity_custom',
					...
					$ratings=$this->Locations_model->getLocation_rating($this->input->post('location_id'));

						$this->db->set('location_ratings', $ratings);
	...
	private function validateForm() {
		...
		$this->form_validation->set_rules('sale_id', 'lang:label_sale_id', 'xss_clean|trim|required|callback__check_sale_id');
		...
		$this->form_validation->set_rules('review_text', 'lang:label_text', 'xss_clean|trim|min_length[2]|max_length[1028]');
