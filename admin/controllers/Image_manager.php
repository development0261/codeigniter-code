...
class Image_manager extends Admin_Controller {
        ...
        foreach($files as $k => $file) {
            try{
                ...
                if ($file['type'] === 'img') {
                    $thumb_type = 'thumb';
                    if ($file_name !== '' ) {
                       $thumb_url = image_url($root_folder . $sub_folder . $file_name);
                       // $thumb_url = $this->Image_tool_model->resize($sub_folder . $file_name, '320', '240');
    ...
    private function setTemplateTags($popup) {
        if ($popup == 'iframe') {
            ...
            $this->template->setFavIcon('images/favicon.png', 'shortcut icon', 'image/png');
