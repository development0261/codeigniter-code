...
class Settings extends Admin_Controller {
	...
	public function index() {
		...
		if (!isset($data['digits_mobile'])) {
            $this->config->set_item('digits_mobile', '29');
        }
	...
	public function send_test_email() {
		...
		if (empty($json)) {
			...
			$this->email->message('This is a test email. If you\'ve received this, it means emails are working in SpotnEat.');
	...
	private function _updateSettings() {		
        if ($this->validateForm() === TRUE) {
	        $update = array(
		        ...
		        'digits_mobile'					   => $this->input->post('digits_mobile'),
		        ...
		        'guest_reservation_email'          => $this->input->post('guest_reservation_email'),
			...
			if(!empty($_FILES['eat_right_pdf']['tmp_name'])){
				$file_name  = pathinfo($_FILES['eat_right_pdf']['name'], PATHINFO_FILENAME);
				$file_name  = preg_replace('/[^A-Za-z0-9\-]/', '-', $file_name).'-'.str_pad(rand(0, pow(10, 5)-1), 5, '0', STR_PAD_LEFT);
				
				$file_ext  = pathinfo($_FILES['eat_right_pdf']['name'], PATHINFO_EXTENSION);
				$image_name= $file_name.'.'.$file_ext;

				$target_dir = '../admin/views/uploads/trainers/eat_right_pdf/'.$image_name;
				move_uploaded_file($_FILES['eat_right_pdf']['tmp_name'], $target_dir);
				if(!empty($this->input->post('eat_right_pdf_old_value'))){
					unlink('../admin/views/uploads/trainers/eat_right_pdf/'.$this->input->post('eat_right_pdf_old_value'));
				}
				// Update Settings
				$update['EAT_RIGHT_PDF'] = $image_name;
			}
	...
	private function validateForm() {
		...
		$this->form_validation->set_rules('page_limit', 'lang:label_page_limit', 'xss_clean|trim|integer');
		...
		$this->form_validation->set_rules('menus_page_limit', 'lang:label_menu_page_limit', 'xss_clean|trim|integer');
		...
		//$this->form_validation->set_rules('guest_reservation_email[]', 'lang:label_reservation_email', 'xss_clean|trim|required|alpha');
		...
		$this->form_validation->set_rules('rewards_value', 'lang:label_rewards_value', 'xss_clean|trim|integer');
