...
class Extensions_model extends CI_Model {
	...
	private function extensions() {
		...
		foreach ($installed_extensions as $extension_path) {
			...
			// echo '<pre>'; print_r($extension_path); echo '</pre>';
			$extension = array();
			...
			$extension_type = $extension_path['name'];
			$extension = $extension_path['ext_data'];
			$extensions[$extension_type] = $extension;
		}

		// print_r($extensions['twilio_module'][0]);exit;
	...
	public function getList($filter = array()) {
		...
		foreach ($this->extensions() as $type => $extensions) { echo '<pre>'; print_r($type); echo '</pre>';
		exit;
	...
	public function getExtension($name = '', $filter = array('filter_status' => '1')) {
		...
		if ( ! empty($name)) { 
			...
			if ($extensions AND is_array($extensions)) {
				...
				foreach ($extensions[$name] as $key => $value) { 
