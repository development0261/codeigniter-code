...
class Addresses_model extends CI_Model {
	...
	public function saveAddress($customer_id = FALSE, $address_id = FALSE, $address = array()) {
		...
		if (isset($address['default_address'])) {
			$this->db->set('default_address', $address['default_address']);
		}
		...
		if (isset($address['default_address'])) {
			if($address['default_address']=='on'){
				$this->custUpdate($customer_id,$address_id,$address['default_address']);
			} 
			$this->db->set('default_address', $address['default_address']);
		}
	...
	public function custUpdate($customer_id,$address_id,$default_address) {
	  ...
	  $this->db->where('customer_id',$customer_id);
	  $this->db->where('address_id',$address_id);
	  $this->db->update('addresses',array('default_address'=>$default_address));
	  return true;
	}
}
