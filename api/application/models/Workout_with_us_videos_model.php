 ...
 */
class Workout_with_us_videos_model extends CI_Model {
	...
	*/
	...
	{		
		$result = $this->db->query("SELECT `rv`.*, `vm`.`module_name`, `vm`.`module_image`
						  FROM `yvdnsddqu_workout_with_us_videos` `rv` 
	...
	*/
	public function getWorkoutVideoModules($workout_video_id = '')
	{
		...
		$this->db->join('workout_with_us_videos wwu','wwu.workout_video_module_id = vm.workout_video_module_id','inner');
