...
class workout_with_us_videos extends REST_Controller {
    ...
    {
        $categoriesList = $this->Workout_with_us_videos_model->getCategoriesList();
    ...
    public function list_get($workout_video_categoryid = '') 
    {
        ...
        */
        $workoutVideos = $this->Workout_with_us_videos_model->getVideosList($workout_video_categoryid);
        if(!empty($workoutVideos))
        {
            foreach($workoutVideos as $key=>$value)
            {
                $workoutVideos[$key]['banner_image'] = base_url().'admin/views/uploads/workoutwithusvideos/'.$value['banner_image'];
            }
    ...
    public function details_get($workout_video_id = '') 
    {
        ...
        else
        {
            ...
            */
            $workoutVideoDetails = $this->Workout_with_us_videos_model->getWorkoutVideoDetails($workout_video_id);
            if(!empty($workoutVideoDetails))
            {                
                $workoutVideoDetails = $workoutVideoDetails[0];                
                $workoutVideoDetails['banner_image'] = base_url().'admin/views/uploads/workoutwithusvideos/'.$workoutVideoDetails['banner_image'];
                $workoutVideoDetails['image'] = base_url().'admin/views/uploads/workoutwithusvideos/'.$workoutVideoDetails['image'];
                $workoutVideoDetails['tags'] = json_decode($workoutVideoDetails['tags']);
                
                /*
                * Related free videos
                */
                $workoutVideoSeries = $this->Workout_with_us_videos_model->getWorkoutFreeVideos($workout_video_id);                
                if(!empty($workoutVideoSeries))
                {          
                    foreach($workoutVideoSeries as $key=>$value)
                    {                   
                        $workoutVideoSeries[$key]['image'] = base_url().'admin/views/uploads/workoutwithusvideos/'.$value['image'];
    ...
    public function videoseries_get() 
    {
        ...
        $workoutVideoSeries = $this->Workout_with_us_videos_model->getWorkoutVideoSeries();
        ...
        if(!empty($workoutVideoSeries))
        {
            ...
            */
            ...
            $videoSeriesInfo['workout_with_us_videos'] = $related_videos;
            
            $output    = array('result'  => $videoSeriesInfo, 
            'message'  => 'Workout with us video Details Fetched');            
        }            
        else
        {
    ...
    public function modules_get() 
    {
        ...
        {                   
            ...
            */
            $workoutVideoModules = $this->Workout_with_us_videos_model->getWorkoutVideoModules($video_id);                
    ...
    public function schedules_get() 
    {
        ...
        {                   
            ...
            */
            $workoutVideoSchedules = $this->Workout_with_us_videos_model->getWorkoutVideoSchedules($video_id);                
    ...
    {              
        ...
        */
        $check_customer = $this->Workout_with_us_videos_model->checkCustomer($customer_id);
        if($check_customer > 0)
        {
            $userData = array();
            ...
            $onboarding_id = $this->Workout_with_us_videos_model->saveOnboardingProcess($onboarding_id , $userData);
    ...
    {   
        ...
        */
        ...
        if($check_customer > 0)
        {
            $check_onboardtype = $this->Workout_with_us_videos_model->checkOnboard($onboarding_id);
            ...
            if($check_onboardtype > 0)
            {
                ...
                $updateCount = $this->Workout_with_us_videos_model->updateOnboardingProcess($userData, $onboarding_id);
    ...
    public function onboardingdetails_get() 
    {
        ...
        else
        {
            ...
            * Fetch Workout videos details
            */
            $onboardingdetails = $this->Workout_with_us_videos_model->onboardingdetails($customer_id);            
    ...
    public function subscriptiondetails_get() 
    {
        ...
        {
            $videoSeriesInfo = array();
            $getSubscriptionDetails = $this->Workout_with_us_videos_model->getSubscriptionDetails($email);
